 ...
 */
package org.l2junity.scripts.custom.listeners;
...
public class ListenerTest extends AbstractNpcAI
{
	...
	private ListenerTest()
	{
		...
		{
			_logger.info(event.getActiveChar() + " OnPlayerDlgAnswer: Answer: " + event.getAnswer() + " MessageId: " + event.getMessageId());
		}, this));
	}
	...
	{
		_logger.info(event.getClass().getSimpleName() + " invoked attacker: " + event.getAttacker() + " target: " + event.getTarget() + " damage: " + event.getDamage() + " skill: " + event.getSkill());
	}
	...
	{
		_logger.info(event.getClass().getSimpleName() + " invoked attacker: " + event.getAttacker() + " target: " + event.getTarget());
	}
	...
	{
		_logger.info("The siege of " + event.getSiege().getCastle().getName() + " (" + event.getSiege().getCastle().getResidenceId() + ") has started!");
	}
	...
	{
		_logger.info("Item [" + event.getItem() + "] has been created actor: " + event.getActiveChar() + " process: " + event.getProcess() + " reference: " + event.getReference());
	}
	
	/**
	 * This method will be invoked as soon as Power Shot (56) skill is used on creatire.<br>
	 * This listener is registered into individual skills container.
	 * @param event
	 */
	@RegisterEvent(EventType.ON_CREATURE_SKILL_USED)
	@RegisterType(ListenerRegisterType.SKILL)
	@Id(56)
	private void onSkillUsed(OnCreatureSkillUsed event)
	{
		_logger.info("Skill [" + event.getSkill() + "] has been used actor: " + event.getCaster() + " target: " + event.getTarget() + " isSimultaneously: " + event.isSimultaneously());
	}
	...
	{
		_logger.info("Player: " + event.getActiveChar() + " has logged in!");
	}
	...
	private TerminateReturn onPlayerDeath(OnCreatureDeath event)
	{
		...
		{
			_logger.info("Player: " + event.getTarget() + " was prevented from dying!");

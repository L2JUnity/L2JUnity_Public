 ...
 */
package org.l2junity.scripts.ai.individual.PrimevalIsle.Sailren;
...
public final class Sailren extends AbstractNpcAI
{
	...
	private Sailren()
	{
		...
		final long remain = GlobalVariablesManager.getInstance().getLong(GlobalVariablesManager.SAILREN_RESPAWN_VAR, 0) - System.currentTimeMillis();
		if (remain > 0)
		{
			STATUS = Status.DEAD;
		...
		if (zone.isCharacterInZone(killer))
		{
			switch (npc.getId())
			{
				case SAILREN:
				{
					...
					GlobalVariablesManager.getInstance().set(GlobalVariablesManager.SAILREN_RESPAWN_VAR, System.currentTimeMillis() + respawnTime);
	...
	public boolean unload(boolean removeFromList)
	{
		if (STATUS == Status.IN_FIGHT)
		{
			_logger.warn("Script is being unloaded while Sailren is active, clearing zone.");
			notifyEvent("TIME_OUT", null, null);
		}

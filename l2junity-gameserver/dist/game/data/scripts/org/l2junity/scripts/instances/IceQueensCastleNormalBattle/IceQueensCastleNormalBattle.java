 ...
 */
package org.l2junity.scripts.instances.IceQueensCastleNormalBattle;
			...
			if (world != null)
			{
				...
				switch (event)
				{
					...
					case "LEADER_DASH":
					{
						...
						if (getRandomBoolean() && !npc.isCastingNow(SkillCaster::isAnyNormalType) && (mostHated != null) && !mostHated.isDead() && (npc.distance3d(mostHated) < 1000))
						{
			...
			switch (npc.getId())
			{
				case FREYA_THRONE:
				{
					...
					else
					{
						...
						final Creature mostHated = ((Attackable) npc).getMostHated();
						final boolean canReachMostHated = (mostHated != null) && !mostHated.isDead() && (npc.distance3d(mostHated) <= 800);
						
						if (getRandom(10000) < 3333)
						{
							if (getRandomBoolean())
							{
								if ((npc.distance3d(attacker) <= 800) && SkillCaster.checkUseConditions(npc, ICE_BALL.getSkill()))
								{
									npc.setTarget(attacker);
						...
						else if (getRandom(10000) < 800)
						{
							if (getRandomBoolean())
							{
								if ((npc.distance3d(attacker) <= 800) && SkillCaster.checkUseConditions(npc, SUMMON_ELEMENTAL.getSkill()))
								{
									npc.setTarget(attacker);
				...
				case GLAKIAS_HARD:
				{
					...
					{
						final Creature mostHated = ((Attackable) npc).getMostHated();
						final boolean canReachMostHated = (mostHated != null) && !mostHated.isDead() && (npc.distance3d(mostHated) < 1000);

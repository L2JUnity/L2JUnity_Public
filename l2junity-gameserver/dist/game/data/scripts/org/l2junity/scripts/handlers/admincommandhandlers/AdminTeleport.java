...
public class AdminTeleport implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, PlayerInstance activeChar)
	{
		...
		else if (command.startsWith("admin_walk"))
		{
			...
			catch (Exception e)
			{
				...
				{
					LOGGER.info("admin_walk: " + e);
				}
			}
		}
		else if (command.startsWith("admin_come_to_me"))
		{
			final WorldObject target = activeChar.getTarget();
			if ((target != null) && target.isCreature())
			{
				String val = command.substring(17);
				if ("1".equals(val)) // I guess its for setting NPC to run :)
				{
					((Creature) target).setRunning();
				}
				((Creature) target).getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, activeChar.getLocation());
			}
			else
			{
		...
		{
			int intVal = 150;
			double x = activeChar.getX(), y = activeChar.getY(), z = activeChar.getZ();
			try
			{
	...
	{
		final int x = (int) activeChar.getX();
		final int y = (int) activeChar.getY();
		final int z = (int) activeChar.getZ();
	...
	private void recallNPC(PlayerInstance activeChar)
	{
		...
		if ((obj instanceof Npc) && !((Npc) obj).isMinion() && !(obj instanceof L2RaidBossInstance) && !(obj instanceof L2GrandBossInstance))
		{
			...
			try
			{
				...
				if (GeneralConfig.DEBUG)
				{
					LOGGER.debug("Spawn at X=" + spawn.getX() + " Y=" + spawn.getY() + " Z=" + spawn.getZ());
					LOGGER.warn("GM: " + activeChar.getName() + "(" + activeChar.getObjectId() + ") moved NPC " + target.getObjectId());
				}
			}
			catch (Exception e)
			{
	...
	{
		AdminCommandHandler.getInstance().registerHandler(new AdminTeleport());
	}
}
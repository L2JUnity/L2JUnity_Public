 ...
 */
package org.l2junity.scripts.instances.CrystalCaverns;
 ...
 * @author St3eT
 */
public final class CrystalCavernsSteamCorridor extends AbstractInstance
{
	// NPCs
				...
				case VICIOUS_SWORDSMAN:
				{
					if (npcParams.getInt("last_checker", 0) == 1)
					{
						...
						if (currentKillCount >= killTarget)
						{
							...
							{
								_logger.warn("Cannot find teleport zone for Crystal Cavern: Steam Corridor instance!!!");
							}
						}
					}
					break;
			...
			switch (npc.getId())
			{
				case PLAYER_DETECTOR:
				{
					if (npc.isScriptValue(0))
					{
						...
						{
							final Npc trap = addSpawn(((npcParams.getInt("MobType", 0) == 0) ? TRAP_1 : TRAP_2), npc.getRandomPosition(50, 250), false, 0, false, instance.getId());

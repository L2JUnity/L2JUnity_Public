 ...
 */
package org.l2junity.scripts.handlers.actionhandlers;
...
public class L2SummonAction implements IActionHandler
{
	...
	public boolean action(PlayerInstance activeChar, WorldObject target, boolean interact)
	{
		...
		{
			if (!activeChar.isInRadius3d(target, Npc.INTERACTION_DISTANCE))
			{
		...
		else if (interact)
		{
			...
			else
			{
				...
				activeChar.sendPacket(ActionFailed.STATIC_PACKET);
				if (target.isInRadius2d(activeChar, 150))
				{
					activeChar.updateNotMoveUntil();
				}
				else
	...
	{
		ActionHandler.getInstance().registerHandler(new L2SummonAction());
	}
}
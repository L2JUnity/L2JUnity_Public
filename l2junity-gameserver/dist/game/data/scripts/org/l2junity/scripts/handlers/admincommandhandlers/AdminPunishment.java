...
public class AdminPunishment implements IAdminCommandHandler
{
					...
					switch (subcmd)
					{
						...
						case "player":
						{
							...
							if (content != null)
							{
								...
								content = content.replaceAll("%ip%", target.getIPAddress("N/A"));
								activeChar.sendPacket(new NpcHtmlMessage(0, 1, content));
							}
							else
							{
			...
			case "admin_punishment_add":
			{
				...
				else if (affect == PunishmentAffect.IP)
				{
					try
					{
						...
						else if (NetworkConfig.GAME_SERVER_HOSTS.contains(addr.getHostAddress()))
						{
	...
	{
		AdminCommandHandler.getInstance().registerHandler(new AdminPunishment());
	}
}
 ...
 */
package org.l2junity.scripts.instances.SeedOfDestruction;
...
public final class Stage1 extends AbstractInstance implements IGameXmlReader
{
	...
	public void load()
	{
		...
		_logger.info("[Seed of Destruction] Loaded " + _spawnZoneList.size() + " spawn zones data.");
	}
	...
	private void spawnFlaggedNPCs(Instance world, int flag)
	{
		for (SODSpawn spw : _spawnList.get(flag))
		{
			if (spw.isZone)
			{
				...
				{
					_logger.info("[Seed of Destruction] Missing zone: " + spw.zone);
				}
			...
			switch (world.getStatus())
			{
				...
				default:
				{
					if (world.getStatus() >= 7)
					{
						...
						{
							final Npc tiat = world.getNpc(TIAT);
							if ((tiat != null) && tiat.isMonster())
							{
								addMinion(tiat.asMonster(), TIAT_GUARD);
							}
						}
					}
				}

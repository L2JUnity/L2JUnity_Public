 ...
 */
package org.l2junity.scripts.instances;
...
public abstract class AbstractInstance extends AbstractNpcAI
{
	...
	{
		return (instance != null) && ArrayUtil.contains(_templateIds, instance.getTemplateId());
	}
	...
	protected final void enterInstance(PlayerInstance player, Npc npc, int templateId)
	{
		...
		else
		{
			...
			if (template == null)
			{
				_logger.warn("Player {} ({}) wants to create instance with unknown template id {}!", player.getName(), player.getObjectId(), templateId);
				return;
			}
			...
			if (enterGroup == null)
			{
				_logger.warn("Instance {} ({}) has invalid group size limits!", template.getName(), templateId);
				return;
			}
	...
	protected void teleportPlayerIn(PlayerInstance player, Instance instance)
	{
		...
		{
			_logger.warn("Missing start location for instance {} ({})", instance.getName(), instance.getId());
		}
	}

...
public class Loto implements IBypassHandler
{
	...
	public static final void showLotoWindow(PlayerInstance player, Npc npc, int val)
	{
		...
		else if (val == 22) // 22 - selected ticket with 5 numbers
		{
			...
			long price = GeneralConfig.ALT_LOTTERY_TICKET_PRICE;
			...
			iu.addItem(item);
			ItemInstance adenaupdate = player.getInventory().getAdenaInstance();
		...
		html.replace("%ticket_price%", "" + GeneralConfig.ALT_LOTTERY_TICKET_PRICE);
		html.replace("%prize5%", "" + (GeneralConfig.ALT_LOTTERY_5_NUMBER_RATE * 100));
		html.replace("%prize4%", "" + (GeneralConfig.ALT_LOTTERY_4_NUMBER_RATE * 100));
		html.replace("%prize3%", "" + (GeneralConfig.ALT_LOTTERY_3_NUMBER_RATE * 100));
		html.replace("%prize2%", "" + GeneralConfig.ALT_LOTTERY_2_AND_1_NUMBER_PRIZE);
	...
	{
		BypassHandler.getInstance().registerHandler(new Loto());
	}
}
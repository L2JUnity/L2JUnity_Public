 ...
 */
package org.l2junity.scripts.ai.individual.ForgeOfTheGods;
...
public final class Valakas extends AbstractNpcAI
{
	...
	private Valakas()
	{
		...
		if (status == DEAD)
		{
			...
			else
			{
				...
				valakas.getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
			}
		}
		else
		{
	...
	private void callSkillAI(Npc npc)
	{
		...
		if (_actualVictim == null)
		{
			if (getRandom(10) == 0)
			{
				final double posX = npc.getX() + getRandom(-1400, 1400);
				final double posY = npc.getY() + getRandom(-1400, 1400);

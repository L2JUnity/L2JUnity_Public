...
public final class DenOfEvil extends AbstractNpcAI
{
	...
	public String onSpawn(Npc npc)
	{
		...
		if (zone == null)
		{
			_logger.warn("NPC " + npc + " spawned outside of L2EffectZone, check your zone coords! X:" + npc.getX() + " Y:" + npc.getY() + " Z:" + npc.getZ());
			return null;
		}
		...
		{
			ThreadPool.schedule(new KashaDestruction(zone), 2 * 60 * 1000l, TimeUnit.MILLISECONDS);
	...
	{
		ThreadPool.schedule(new RespawnNewEye(npc.getLocation()), 15000, TimeUnit.MILLISECONDS);
		EffectZone zone = ZoneManager.getInstance().getZone(npc, EffectZone.class);
		if (zone == null)
		{
			_logger.warn("NPC " + npc + " killed outside of L2EffectZone, check your zone coords! X:" + npc.getX() + " Y:" + npc.getY() + " Z:" + npc.getZ());
			return null;
		}
		int skillId = getSkillIdByNpcId(npc.getId());
		int skillLevel = zone.getSkillLevel(skillId);

 ...
 */
package org.l2junity.scripts.quests.Q00453_NotStrongEnoughAlone;
...
public final class Q00453_NotStrongEnoughAlone extends Quest
{
	...
	private void increaseKill(PlayerInstance player, Npc npc)
	{
		...
		{
			if (ArrayUtil.contains(MONSTER1, npcId) && st.isCond(2))
			{
				if (npcId == MONSTER1[4])
				{
					npcId = MONSTER1[0];
			...
			else if (ArrayUtil.contains(MONSTER2, npcId) && st.isCond(3))
			{
				if (npcId == MONSTER2[3])
				{
					npcId = MONSTER2[0];
			...
			else if (ArrayUtil.contains(MONSTER3, npcId) && st.isCond(4))
			{
				if (npcId == MONSTER3[3])
				{
					npcId = MONSTER3[0];

 ...
 */
package org.l2junity.scripts.instances.IstinaCavern;
...
public final class IstinaCavern extends AbstractInstance
{
		...
		if (isInInstance(instance))
		{
			...
			switch (event)
			{
				case "DEATH_TIMER":
				{
					...
					if ((mostHated != null) && npc.isInRadius2d(mostHated, 15000))
					{
				...
				case "DEATH_CHECK_TIMER":
				{
					...
					if ((mostHated != null) && npc.isInRadius2d(mostHated, 15000) && mostHated.isInCategory(CategoryType.TANKER_GROUP) && mostHated.isAffectedBySkill(death1))
					{
						addSkillCastDesire(npc, mostHated, death2, 23);
					}
					break;
				...
				case "LOW_ERUPTION_TIMER":
				{
					...
					{
						final Creature target = istina.getHateList().stream().sorted((o1, o2) -> (int) o1.distance3d(o2)).findFirst().orElse(null);
						if (target != null)
						{
							final Npc eruption = addSpawn(INVISIBLE_NPC, target.getRandomPosition(50, 50), false, 0, false, instance.getId());
		...
		if ((skill != null) && isInInstance(instance))
		{
			...
			else
			{
				...
				else if (skillId == ISTINA_ERUPTION_SKILL.getSkillId())
				{
					...
					{
						final Npc eruption = addSpawn(INVISIBLE_NPC, character.getRandomPosition(150, 150), false, 0, false, instance.getId());
	...
	private int getChargedPercent(int score, boolean isExtreme)
	{
		...
		{
			charged = (int) ((score * 100L) / (isExtreme ? PERFECT_SCORE_2 : PERFECT_SCORE_1));
		}
		return charged;
	}
	...
	private PlayerInstance setPlayerRewardInfo(Npc npc)
	{
		...
		for (AggroInfo info : ((Attackable) npc).getAggroList().values())
		{
			...
			if (attacker != null)
			{
				...
				if (damage > 1)
				{
					if (!Util.checkIfInRange(PlayerConfig.ALT_PARTY_RANGE, npc, attacker, true))
					{
						continue;
					}

 ...
 */
package org.l2junity.scripts.custom.events.Race;
...
public final class Race extends Event
{
	...
	public boolean eventStart(PlayerInstance eventMaker)
	{
		...
		// Schedule Event end
		_eventTask = ThreadPool.schedule(() -> StartRace(), _time_register * 60 * 1000, TimeUnit.MILLISECONDS);
	...
	protected void StartRace()
	{
		...
		for (PlayerInstance player : _players)
		{
			...
			{
				if (player.isInRadius2d(_npc, 500))
				{
		...
		_eventTask = ThreadPool.schedule(() -> timeUp(), _time_race * 60 * 1000, TimeUnit.MILLISECONDS);
	}

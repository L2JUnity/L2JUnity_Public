...
public class AdminFightCalculator implements IAdminCommandHandler
{
	...
	private void handleShow(String params, PlayerInstance activeChar)
	{
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			boolean _crit1 = Formulas.calcCrit(npc1.getCriticalHit(), npc1, npc2, null);
			if (_crit1)
			{
				crit1++;
			}
			...
			if (!_miss1)
			{
				double _dmg1 = Formulas.calcAutoAttackDamage(npc1, npc2, _shld1, _crit1, false);
				dmg1 += _dmg1;
				npc1.abortAttack();
			}
		}
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			boolean _crit2 = Formulas.calcCrit(npc2.getCriticalHit(), npc2, npc1, null);
			if (_crit2)
			{
				crit2++;
			}
			...
			if (!_miss2)
			{
				double _dmg2 = Formulas.calcAutoAttackDamage(npc2, npc1, _shld2, _crit2, false);
				dmg2 += _dmg2;
				npc2.abortAttack();
			}
		}
		...
		int hp1 = (int) ((npc1.getStat().getValue(DoubleStat.REGENERATE_HP_RATE) * 100000) / Formulas.getRegeneratePeriod(npc1));
		
		double maxHp2 = npc2.getMaxHp();
		int hp2 = (int) ((npc2.getStat().getValue(DoubleStat.REGENERATE_HP_RATE) * 100000) / Formulas.getRegeneratePeriod(npc2));
	...
	{
		AdminCommandHandler.getInstance().registerHandler(new AdminFightCalculator());
	}
}
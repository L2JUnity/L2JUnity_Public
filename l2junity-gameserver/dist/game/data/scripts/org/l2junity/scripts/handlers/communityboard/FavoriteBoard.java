...
public class FavoriteBoard implements IParseBoardHandler
{
	...
	public boolean parseCommunityBoardCommand(String command, PlayerInstance activeChar)
	{
		...
		if (command.startsWith("_bbsgetfav"))
		{
			...
			{
				LOGGER.warn(FavoriteBoard.class.getSimpleName() + ": Couldn't load favorite links for player " + activeChar.getName());
			}
		}
		else if (command.startsWith("bbs_add_fav"))
		{
			...
			if (bypass != null)
			{
				...
				{
					LOGGER.warn(FavoriteBoard.class.getSimpleName() + ": Couldn't add favorite link " + bypass + " for player " + activeChar.getName());
				}
			}
		}
		else if (command.startsWith("_bbsdelfav_"))
		{
			...
			{
				LOGGER.warn(FavoriteBoard.class.getSimpleName() + ": Couldn't delete favorite link ID " + favId + " for player " + activeChar.getName());
			}
		}
		return true;
	}
	...
	{
		CommunityBoardHandler.getInstance().registerHandler(new FavoriteBoard());
	}
}
 ...
 */
package org.l2junity.scripts.instances.Kimerian;
...
public final class KimerianCommon extends AbstractInstance
{
		...
		if (isInInstance(instance))
		{
			if (npc.getId() == KIMERIAN)
			{
				if ((npc.getCurrentHpPercent() <= 50) && npc.getVariables().getBoolean("CAN_ACTIVATE_INVUL", true))
				{
					...
					for (int i = 0; i < 5; i++)
					{
						final Npc ghost = addSpawn(KIMERIAN_GHOST, npc.getX(), npc.getY(), npc.getZ(), npc.calculateHeadingTo(player), false, 0, false, instance.getId());
		...
		if (isInInstance(instance))
		{
			switch (npc.getId())
			{
				...
				case KIMERIAN:
				{
					...
					final Location loc = kimerian.getRandomPosition(500, 500);
	...
	private void spawnHollow(Npc npc, PlayerInstance player, boolean isHollow)
	{
		...
		if (isInInstance(instance))
		{
			if (isHollow)
			{
				final Npc kimerian = addSpawn(KIMERIAN_HOLLOW, npc.getX(), npc.getY(), npc.getZ(), npc.calculateHeadingTo(player), false, 0, false, instance.getId());
			...
			{
				final Npc kimerian = addSpawn(KIMERIAN_HOLLOW_2, npc.getX(), npc.getY(), npc.getZ(), npc.calculateHeadingTo(player), false, 0, false, instance.getId());
				kimerian.setTargetable(false);
